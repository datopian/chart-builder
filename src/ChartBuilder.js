      ...
      render={({ values, setFieldValue }) => (
        <Form className="bg-white text-sm">
          <div className="flex flex-wrap -mx-3 mb-2">
            <div className="w-full px-3 mb-4">
              ...
              <div className="relative mt-1">
                <Field name="chartType" component="select" placeholder="Chart type" required className="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-2 px-2 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
            ...
            <div className="w-full px-3 mb-4">
              <label htmlFor="xAxis" className="text-xs font-bold uppercase text-gray-700 mb-1">Group column</label>
              <div className="relative mt-1">
                <Field name="xAxis" component="select" placeholder="Field for X axis" required className="appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-2 px-2 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
            ...
            <div className="w-full  px-3 mb-3">
              <div className="flex flex-col">
                <label htmlFor="yAxis" className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-1">Series</label>
                <Field name="yAxis" placeholder="Field for Y axis" className="mb-4 mr-4">
                  {({ field, form }) => (
                    fields.map((field, index) => {
                      return (
                        <label key={`yAxis${index}`} className="pr-4 text-gray-700 ">
                          <input
                            type="checkbox"
                            name="yAxis"
                            value={field.name}
                            ...
                            onChange={() => {
                              if (values.yAxis.includes(field.name)) {
                                ...
                                const nextValue = values.yAxis.concat(
                                  field.name);
                                setFieldValue('yAxis', nextValue);
                              }
                            }}
                            className="mr-2 leading-tight pl-2 "
                          />
                          {field.name}
                        </label>
                      );
          ...
          <div className="flex justify-center ">
            <button type="submit" className="text-xs bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
              Add view
            </button>
          </div>
        </Form>
